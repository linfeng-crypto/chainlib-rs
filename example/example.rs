...
impl Signer {
    ...
    fn get_transaction(
        ...
        builder.add_transfer(transfer_amount, transfer_denom, to_address)?;
        let transaction = builder.build(account_number, sequence, SyncMode::Sync)?;
        Ok(transaction)
    }
...
async fn main() {
    ...
    let signer = Signer::new(words, None, chain_id, memo, base_api_url).unwrap();
